...
if __name__ == "__main__":
    ...
    r = requests.post('http://0.0.0.0:3456/api/v1/users', data={
        'email': user_email, 'password': user_pwd, 'first_name': 'John',
        'last_name': 'Doe'})
    if r.status_code != 200:
        print(f"account registration failed: {r.status_code}")
        exit(1)

    r = requests.post('http://0.0.0.0:3456/api/v1/auth_session/login', data={ 'email': user_email, 'password': user_pwd })
    if r.status_code != 200:
        print("login failed: Wrong status code: {}".format(r.status_code))
        exit(1)
